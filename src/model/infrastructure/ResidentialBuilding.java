...
public class ResidentialBuilding implements Rescuable, Simulatable 
{
	...
	public String toString() {
		String s = "Location of the Building is " + this.getLocation().getX() + " " + this.getLocation().getY() + "\n" + "the Structural Integrity is "
			    + structuralIntegrity +"\n"+ "the Fire Damage is " + fireDamage + "\n" +"the gas level is " + gasLevel + "\n";
		for(int i =0; i < this.getOccupants().size();i++) {
		s += "Occupant is " + this.getOccupants().get(i).toString()+"\n";	
		}
//	   s+=this.getDisaster().toString();
	return s;				
	}
}
